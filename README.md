...
- 当前开发构建：`测试版 v0.3.0 (25)`，加入本地录制/在线课堂工作模式、零成本课堂联动、一键联动自检、本地课堂二维码和 Mac 端新建课堂。
...
- 支持一键课堂联动自检：仅在零成本 `mock` 模式下自动完成短时录制、开课、结束、MP4 保存和结果定位。
- 支持在本机生成课堂二维码；二维码内容就是学员链接，不依赖第三方服务或云端调用。
- 支持直接在 Mac 软件中创建并选择课堂，创建完成后立即得到独立的学员链接和二维码。
...
- 完成在线课堂开始/结束并同步保存本地 MP4 的人工回归。
- 在不产生云费用的前提下继续完善课堂管理、诊断和 Mac 原生联动。
- 需要真实公网音视频时，再关闭零成本保护并开启一次受控的 TRTC 免费额度测试。
- 真实媒体链路通过后，再制作 `0.3.0` 测试安装包并进行 iPhone/安卓微信双机测试。
